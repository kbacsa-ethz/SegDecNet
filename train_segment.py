...
from dataset import KolektorDataset, TokaidoTextureDataset
...
dataSetRoot = '/home/kiran/Tokaido_dataset'
df_puretex_train = pd.read_csv(os.path.join(dataSetRoot, 'files_puretex_train.csv'),
...
for epoch in range(opt.begin_epoch, opt.end_epoch):
    ...
    if opt.need_test and epoch % opt.test_interval == 0 and epoch >= opt.test_interval:
        ...
        for i, testBatch in enumerate(testloader):
            ...
            im = Image.fromarray(imgTest.data)
            seg = Image.fromarray(segTest.data)

            experiment.log_figure(figure=fig, figure_name="%s_img_%d".format(epoch))
